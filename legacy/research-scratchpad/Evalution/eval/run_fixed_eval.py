...
def analyze_results():
    ...
    try:
        ...
        metrics_cols = ['system', 'confidence', 'latency_ms', 'EM', 'F1', 'Num_Acc@tol', 'Num_MAE', 'ROUGE_L']
        available_cols = [col for col in metrics_cols if col in summary.columns]
        summary_display = summary[available_cols].copy()
        
        # Format for better readability
        for col in ['confidence', 'EM', 'F1', 'Num_Acc@tol', 'ROUGE_L']:
        ...
        if len(summary) > 1:
            ...
            for _, row in summary.iloc[1:].iterrows():
                for col in ['EM', 'F1', 'Num_Acc@tol', 'confidence']:
        ...
        for system in summary['system'].unique():
            ...
            print(f"   Exact Match (EM): {system_summary['EM']:.1%}")
            print(f"   F1 Score: {system_summary['F1']:.4f}")
            print(f"   Numeric Accuracy: {system_summary['Num_Acc@tol']:.1%}")
            print(f"   Mean Absolute Error: {system_summary['Num_MAE']:.3f}")
            print(f"   Average Confidence: {system_summary['confidence']:.4f}")
            print(f"   Average Latency: {system_summary['latency_ms']:.1f} ms")
        ...
        if identical_performance:
            ...
            if summary_scored['Num_MAE'].max() > summary_scored['Num_MAE'].min():
            ...
            weights = {
                'norm_num_acc@tol': 0.30,  # Numeric accuracy most important
                'norm_em': 0.25,       # Exact match important
                'norm_f1': 0.20,       # F1 score
                'norm_confidence': 0.10,     # Confidence
                'norm_latency': 0.10,  # Speed
                'norm_mae': 0.05       # Lower error
            }
            ...
            print("\nRANKING (by composite accuracy score):")
            print("-" * 50)
            
            for i, (_, row) in enumerate(ranking.iterrows(), 1):
                print(f"{i}. 🥇 {row['system']} (Score: {row['composite_score']:.3f})")
                print(f"   • Numeric Accuracy: {row['Num_Acc@tol']:.1%}")
                print(f"   • Exact Match: {row['EM']:.1%}")
        ...
        if summary['Num_Acc@tol'].mean() < 0.8:
        ...
        print(f"• Average latency: {summary['latency_ms'].mean():.1f} ms")
...
def main():
    print("🔬 MULTIMODAL FUSION MODEL EVALUATION & SELECTION")
    print("="*60)
    print("This will evaluate Early, Late, and Hybrid fusion models")
    print("and recommend the best performing model based on accuracy.")
    print("="*60)
    ...
    proceed = input("\nDo you want to run the full evaluation? (y/n): ").lower().strip()
    if proceed != 'y':
    ...
    if run_evaluation():
        ...
        print(f"\n📊 Analyzing results and selecting best model...")
        analyze_results()
    else:

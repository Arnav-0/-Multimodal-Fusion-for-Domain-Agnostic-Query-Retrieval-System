...
async def answer_one_question(query: str, corpus: Dict[str, Any]):
    ...
    is_summary = any(word in query.lower() for word in ["summarize", "summary", "overview", "main points", "key findings", "describe", "explain the", "what is"])
    ...
    if is_summary:
        ...
        prompt = (
            "You are an expert analyst providing DETAILED, COMPREHENSIVE answers.\n\n"
            f"USER QUESTION: {query}\n\n"
            "📚 AVAILABLE INFORMATION (Text, Tables, Charts, Figures):\n\n" + 
            "\n\n" + "="*80 + "\n\n".join(f"\n\n📄 SOURCE {i+1}:\n{s}\n\n{'='*80}" for i, s in enumerate(support_text)) +
            "\n\n🎯 ANSWER REQUIREMENTS:\n\n"
            "1️⃣ **BE THOROUGH:** Provide detailed explanations (minimum 200-300 words for complex questions)\n\n"
            "2️⃣ **EXTRACT ALL DATA:** Include EVERY relevant number, percentage, metric, statistic, or measurement\n\n"
            "3️⃣ **EXPLAIN TABLES/CHARTS:** If tables, charts, or figures are mentioned:\n"
            "   - Describe their content in detail\n"
            "   - List all data points and values\n"
            "   - Explain trends and patterns\n\n"
            "4️⃣ **BE SPECIFIC:** Use exact figures, not vague descriptions. Say '87.3%' not 'high percentage'\n\n"
            "5️⃣ **USE ALL SOURCES:** Integrate information from every source above\n\n"
            "6️⃣ **CITE PAGES:** Reference page numbers for all facts (e.g., 'According to Page 5...')\n\n"
            "7️⃣ **STRUCTURE CLEARLY:** Use paragraphs, bullet points, or subheadings as appropriate\n\n"
            "8️⃣ **CONTEXT AWARENESS:** If the context is insufficient, explain exactly what information is missing\n\n"
            "❌ **AVOID:**\n"
